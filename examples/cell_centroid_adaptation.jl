...
cc_pts_no_prot, c_gs_no_prot, _, _ = line_sites_2d(constraint, mesh_spacing;
    prot_layer=false)
...
G_no_prot = clipped_voronoi_2d(sites_no_prot, bnd)
compute_geometry!(G_no_prot)

# With protection layer
cc_pts_prot, c_gs_prot, prot_pts, p_gs = line_sites_2d(constraint, mesh_spacing;
...
G, pts, F = composite_voronoi_mesh_2d([0.05, 0.05], [1.0, 1.0];
...
for cfg in configs
    local G
    G, _, _ = composite_voronoi_mesh_2d(gS, pdims;
        cell_constraints=w,
