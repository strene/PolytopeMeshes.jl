# Basic PEBI Grid Generation
#
# This example demonstrates how to create basic PEBI grids using
# composite_pebi_grid_2d, which is the Julia equivalent of MRST's
# compositePebiGrid2D.
#
...
## 1. Cartesian-equivalent PEBI grid
# The simplest use case: a rectangular domain with uniform cell sizes.
# This is equivalent to MRST's cartGrid, but produces a Voronoi grid.
dx, dy = 0.25, 0.25   # mesh size in x and y direction
xmax, ymax = 1.0, 1.0 # domain dimensions

G, pts, F = composite_pebi_grid_2d([dx, dy], [xmax, ymax])
compute_geometry!(G)

println("=== Cartesian-equivalent PEBI grid ===")
...
G3, _, _ = composite_pebi_grid_2d([0.15, 0.15], [2.0, 0.5])
compute_geometry!(G3)
